...
/* ──────────────────────────────────────────────────────────── */
/* The 14 Chart Types Taxonomy Required by User                 */
/* ──────────────────────────────────────────────────────────── */
...
export const StatsDetailSpaView: React.FC<StatsDetailSpaViewProps> = ({
    ...
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          setScoreHistory(parsed.map((p: any) => ({
            t: p.t,
            pts: Number(p.pts) || 0,
  ...
  useEffect(() => {
    ...
    setScoreHistory((prev) => {
      ...
      const updated = [...prev, { t: label, pts: score, vol: Math.floor(score * 0.1) }].slice(-30);
  ...
  // Statistical Normal Distribution (Gaussian Curve)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 text-slate-900 select-none font-sans max-w-xl mx-auto w-full px-3 py-3 pb-28">
      ...
      {/* ── 3. HERO CHART CANVAS CONTAINER (Active Chart Rendering) ── */}
      <div className="rounded-3xl bg-gradient-to-br from-[#0088cc] via-[#0077b5] to-[#005f99] p-4 text-white shadow-md border border-blue-400/40 mb-3.5">
        ...
        <div className="w-full h-[210px] flex items-center justify-center pt-1">
          {/* 1. CARTESIAN (Multi-axis Area + Bar + Line) */}
          {activeChart === "cartesian" && (
            <ResponsiveContainer width="100%" height={210}>
              <ComposedChart data={scoreHistory} margin={{ top: 8, right: 4, left: -22, bottom: 0 }}>
                <defs>
              ...
              <ScatterChart margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Scatter data={bubbleData}>
                  {bubbleData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.color} opacity={0.85} stroke="#fff" strokeWidth={1.5} />
                  ))}
                </Scatter>
              </ScatterChart>
            </ResponsiveContainer>
          ...
          {/* 4. GAUGE (Speedometer & Target Capacity Meter) */}
          {activeChart === "gauge" && (
          ...
          {/* 6. HEATMAP & MATRIX (168-Cell 7x24 Matrix) */}
              ...
              <div className="grid grid-cols-12 gap-1 w-full my-auto">
                {Array.from({ length: 48 }, (_, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="h-3.5 rounded-xs transition-all hover:scale-125"
                      style={{ background: bg }}
          ...
          {/* 8. RANKING (Horizontal Bar Comparison) */}
          {activeChart === "ranking" && (
            <ResponsiveContainer width="100%" height={210}>
              <BarChart data={rankingData} layout="vertical" margin={{ top: 6, right: 16, left: 16, bottom: 0 }}>
          ...
          {/* 9. FINANCIAL (OHLC Candlesticks & Price Action) */}
          ...
          {/* 10. GEOSPATIAL & MAPS (World Validator Nodes Network) */}
          ...
          {/* 11. FLOW & NETWORK (Sankey Value Stream) */}
            ...
            <div className="w-full h-full flex flex-col justify-between py-1">
              ...
              <div className="grid grid-cols-3 gap-2 w-full my-auto items-center text-center">
                ...
                <div className="space-y-1 text-[9px]">
                  <div className="p-1 rounded bg-white/10 border border-white/10 font-bold">Tap Mint (40%)</div>
                  <div className="p-1 rounded bg-white/10 border border-white/10 font-bold">Mini-Games (35%)</div>
                  <div className="p-1 rounded bg-white/10 border border-white/10 font-bold">Passive Rig (25%)</div>
                </div>
                {/* Center Hub */}
                <div className="p-2.5 rounded-xl bg-white/20 border-2 border-yellow-300 shadow-md flex flex-col items-center">
                ...
                {/* Right Destinations */}
                <div className="space-y-1 text-[9px]">
                  <div className="p-1 rounded bg-white/10 border border-white/10 font-bold text-emerald-300">TG Wallet (55%)</div>
                  <div className="p-1 rounded bg-white/10 border border-white/10 font-bold text-sky-300">Bakong KHQR (35%)</div>
                  <div className="p-1 rounded bg-white/10 border border-white/10 font-bold text-pink-300">Staking Pool (10%)</div>
                </div>
              </div>
          ...
          {/* 12. STATISTICAL & ENGINEERING (Gaussian Bell Curve) */}
          ...
          {/* 13. SCHEDULING & OPERATIONS (Gantt Schedule) */}
          ...
          {/* 14. ANALYTICS (Executive KPI Dashboard) */}
          {activeChart === "analytics" && (
            <div className="w-full h-full grid grid-cols-3 gap-1.5 items-center py-1">
              {[
                ...
                <div key={k.label} className="bg-white/10 rounded-xl p-2 border border-white/10 text-center shadow-2xs">
      ...
      {/* ── 4. FEATURE | DETAIL TABLE (Requested by User) ── */}
