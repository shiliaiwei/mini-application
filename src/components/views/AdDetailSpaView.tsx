...
export const AdDetailSpaView: React.FC<AdDetailSpaViewProps> = ({
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-4 pt-1 pb-28 animate-fadeIn select-none font-sans text-slate-900">
      ...
      <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm relative overflow-hidden space-y-5 text-center">
        ...
        <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-bold tracking-wide">
        ...
        <div className="flex items-center justify-center py-2">
          <div className="w-36 h-36 rounded-3xl bg-slate-50 border-2 border-slate-100 shadow-md flex items-center justify-center p-3 relative group transition-transform duration-300 hover:scale-105">
        ...
        <div className="space-y-1.5 max-w-md mx-auto">
          ...
          <span className="inline-block mt-1 text-[11px] font-black text-[#0098ea] uppercase tracking-wider bg-sky-50 border border-sky-200 px-3 py-0.5 rounded-full">
          ...
          <div className="space-y-2">
            {partner.keyServices.map((svc, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="p-3 rounded-2xl bg-white hover:bg-slate-50 border border-slate-200 shadow-2xs flex items-center justify-between gap-2 transition-all"
                >
                  ...
                  <button
                    ...
                    disabled={isClaimed}
                    className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-black flex items-center gap-1 transition-all cursor-pointer active:scale-95 ${
                      isClaimed
        ...
        <div className="grid grid-cols-2 gap-2 pt-2">
          <button
            type="button"
            onClick={handlePrev}
            className="py-2.5 px-3 rounded-2xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer active:scale-95"
          >
            <ChevronLeft size={16} />
        ...
        <button
          type="button"
          onClick={onBack}
          className="w-full py-3 rounded-2xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider shadow-sm transition-colors cursor-pointer active:scale-98"
        >
          Exit to Home View
        </button>
      </div>
